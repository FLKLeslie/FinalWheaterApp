...
async function checkCLWeather(apiUrl){6
    try{
...
function weatherImageHandling(condition){
    let imgArray = ["clear","clouds",'drizzle','humidity','mist','rain','snow','storm','wind']
    let returnCondition = ''
        if (imgArray.includes(condition)){
            returnCondition = condition
        }
        else {returnCondition = "mist"}
...
function currentLocationWeather(data){
    ...
    currentLocationImg.src = `Assets/${weatherImageHandling(data.weather[0].main.toLowerCase())}.png`
...
getWeatherData(apiUrl1,apiUrl2,"Bambili")
...
function insertTodaysValues(data){
    ...
    todaysImg.src = `Assets/${weatherImageHandling(data.weather[0].main.toLowerCase())}.png`;
...
function insertForecastValues(data1, data2){
    //inserting images
    forecastImg1.src=  `Assets/${weatherImageHandling(data1.weather[0].main.toLowerCase())}.png`;
    forecastImg2.src=`Assets/${weatherImageHandling(data2.list[8].weather[0].main.toLowerCase())}.png`
    forecastImg3.src=`Assets/${weatherImageHandling(data2.list[16].weather[0].main.toLowerCase())}.png`
    forecastImg4.src=`Assets/${weatherImageHandling(data2.list[24].weather[0].main.toLowerCase())}.png`
    forecastImg5.src=`Assets/${weatherImageHandling(data2.list[32].weather[0].main.toLowerCase())}.png`
    forecastImg6.src=`Assets/${weatherImageHandling(data2.list[39].weather[0].main.toLowerCase())}.png`
    ...
    forecastHeading3.innerHTML = (switchDays(new Date(data2.list[16].dt_txt).getDay()))
    forecastHeading4.innerHTML = (switchDays(new Date(data2.list[24].dt_txt).getDay()))
    forecastHeading5.innerHTML = (switchDays(new Date(data2.list[32].dt_txt).getDay()))
    forecastHeading6.innerHTML = (switchDays(new Date(data2.list[39].dt_txt).getDay()))
// graph data
    let temperatureArr = [data1.main.temp,data2.list[8].main.temp,data2.list[16].main.temp,data2.list[24].main.temp,data2.list[32].main.temp,data2.list[39].main.temp]
